...
class Bots(FastilyBotBase):
    ...
    def dated_file_deletion_notifier(self) -> None:
        ...
        for root_cat, talk_template in json.loads(self.wiki.page_text(self._config_of(6, "Rules"))).items():
            if not (target_cat := next((s for s in self.wiki.category_members(root_cat, NS.CATEGORY) if s.endswith(Bots._DD_TARGET_SUFFIX)), None)):
                log.debug("'%s' does not seem to have a category matching yesterday's date of '%s'", root_cat, Bots._DD_TARGET_SUFFIX)
                continue
